...
class CheckOutService 
{
    ...
    public function store(CheckOutRequest $request)
    {
        try {
            ...
            $dataOrder = [
                ...
                'total_money' => \Darryldecode\Cart\Facades\CartFacade::getTotal() + $fee,
            ...
            foreach(\Darryldecode\Cart\Facades\CartFacade::getContent() as $product){
                $orderDetail = [
                    'order_id' => $order->id,
            ...
            foreach(\Darryldecode\Cart\Facades\CartFacade::getContent() as $product){
                ...
                if($productSize->quantity < $product->quantity) {
                    \Darryldecode\Cart\Facades\CartFacade::update(
                        $product->id,
                        [
                            'quantity' => [
                                'relative' => false,
    ...
    public function paymentMomo() 
    {
        ...
        $amount = \Darryldecode\Cart\Facades\CartFacade::getTotal() + $this->getTransportFee()."";
    ...
    public function getTransportFee()
    {
        try {
            ...
            if ($response->failed()) {
                throw new Exception('Failed to retrieve available services');
            }
            ...
            if ($response->failed()) {
                throw new Exception('Failed to retrieve shipping fee');
            }

            return $response['data']['total'];
        } catch (Exception $e) {
            Log::error('Error in getTransportFee: ' . $e->getMessage());
            return 0; // Return a default fee or handle the error as needed
        }
    }
    ...
    public function payWithMoMo($orderId, $amount, $redirectUrl, $ipnUrl)
    {
        ...
        if (is_null($endPoint)) {
            ...
            throw new Exception('Payment endpoint URL is not configured.');
        }
        ...
        $result = Http::acceptJson()->post($endPoint, $data);
